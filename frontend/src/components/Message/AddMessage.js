...
function AddMessage({ channel_id = '' }) {
  ...
  const submitMessage = () => {
    ...
    if (message.startsWith('/hangman')) {
      axios.post('/hangman/start', { token, channel_id })
        .then(({ data }) => {
          alert('You\'ve started a game of hangman. The game will appear shortly.')
        })
    }

    /*
     * Making a guess in hangman ( any message which starts with /guess)
     */

    if (message.startsWith('/guess')) {
      const re = /\/guess\s+([a-zA-Z\d])/;
      const found = message.match(re);
      if (!found || found.length < 2) {
        alert('Usage: /guess <letter or digit>');
      } else {
        const guess = found[1];
        axios.post('/hangman/guess', { token, channel_id, guess })
          .then(({ data }) => {
            alert("You have guessed: " + guess);
          })
          .catch((err) => { });
      }
    }
